...
export const formatDuration = (ms, lvl = 6) => {
  ...
  return `${yrs ? `${pd(yrs)}y-` : ""}${mths || yrs ? `${pd(mths)}m-` : ""}${days || mths || yrs ? `${pd(days)}d ` : ""
...
export const formatDurationV2 = (seconds) => {
  ...
  if (seconds < 60) {
    formattedDuration += `00:00:${remainingSeconds > 9 ? remainingSeconds : `0${remainingSeconds}`
      }`;
  } else if (seconds < 3600) {
    formattedDuration += `00:${minutes > 9 ? minutes : `0${minutes}`}:${remainingSeconds > 9 ? remainingSeconds : `0${remainingSeconds}`
      }`;
  } else {
    formattedDuration += `${hours > 9 ? hours : `0${hours}`}:${minutes > 9 ? minutes : `0${minutes}`
    ...
    reader.onload = (e) => {
      ...
      img.onload = () => {
        ...
        if (!imgExtensions.includes(fileExtension)) {
          ...
          setErrorState(t('Please make sure that the image size is not larger than 5 MB.'))
        } else {
          setErrorState(null)
        }
      };
