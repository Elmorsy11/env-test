...
const Header = ({ settBtnRef }) => {
  ...
  const handleSignOut = (e) => {
    ...
    if (session[0].user.user.actormode) {
      const actorUser = jwt_decode(session[0]?.user?.user?.parentToken); //JSON.parse(Buffer.from(auth.user.new_token.split('.')[1], 'base64').toString());
      signIn("credentials", {
        user: JSON.stringify({
          new_token: session[0]?.user?.user?.parentToken,
          user: actorUser,
        }),
      });
    } else {
                    ...
                    >
                      ...
                      {session[0].user?.user?.actormode ? (
                        <div>
                          ...
                          {t("Logbackas")}{" "}
                          {
                            jwt_decode(session[0].user.user.parentToken)
                              .username
                          }{" "}
                        </div>
                      ) : (
